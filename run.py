...
class VideoGenerator:
    def __init__(self):
        ...
        self.audio_checkbox = Checkbutton(window, text="Audio", variable=self.audio, onvalue=1, offvalue=0, state=NORMAL)
        self.audio_checkbox.select()
        self.audio_checkbox.grid(column=2, row=1, padx=5, pady=5)
        self.duration_method_text = Label(window, text="Duração do vídeo (s).")
        ...
        self.duration_method_cb["values"] = [
            "-SELECIONAR-",
            "Definir duração total",
            "Definir duração por imagem",
            "Ajustar ao áudio",
        ]
        self.duration_method_cb.current(0)
        self.duration_method_cb.bind("<<ComboboxSelected>>", self.change_duration_method)
        self.fps_text = Label(window, text="FPS")
        self.fps_text.grid(column=4, row=0, padx=5, pady=5)
        self.fps_entry = Entry(window, width=4)
        self.fps_entry.grid(column=4, row=1, padx=5, pady=5)
        self.fps_entry.insert(END, "20")
        self.convert_button = Button(window, text="Gerar video", command=self.makeclip)
        self.convert_button.grid(column=5, row=1, padx=5, pady=5)
    ...
    def makeclip(self):
        ...
        if self.audio.get():
            ...
            video.write_videofile(f"{self.path}/video.mp4", fps=self.fps)
            ...
            if self.duration_method == "Definir duração total":
                ...
                self.processing_text.set(f"Você não habilitou utilização de áudio.")
                do = False
    ...
    def search_images_folder(self):
        ...
        if any(any(file.endswith(img_format) for img_format in self.img_formats) for file in os.listdir(self.path)):
            ...
            if self.img_folder not in os.listdir(self.path):
                os.mkdir(f"{self.path}/{self.img_folder}")
            for file in os.listdir(self.path):
                if any(file.endswith(img_format) for img_format in self.img_formats):
                    shutil.move(f"{self.path}/{file}", f"{self.path}/{self.img_folder}/{file}")
        else:
            for item in os.listdir(self.path):
                if os.path.isdir(f"{self.path}/{item}"):
                    if (
                        all(
                            any(file.endswith(img_format) for img_format in self.img_formats)
                            for file in os.listdir(f"{self.path}/{item}")
                            if os.path.isfile(f"{self.path}/{item}/{file}")
                        )
                        and len(os.listdir(f"{self.path}/{item}")) > 0
                    ):
